    ...
    <button onClick={onClick} style={{
      ...
      <div style={{ padding: "12px 16px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 14, color: C.brand, flexShrink: 0 }}>
            ...
            {visibleCertifications.length > 0 && (
              <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
                {visibleCertifications.map((cert, idx) => (
                  <span key={idx} style={{ fontSize: 11, background: C.bg, padding: "2px 8px", borderRadius: C.r.sm, color: C.muted }}>
                    📜 {cert}
                  </span>
                ))}
              </div>
        ...
        {step === 1 && hasServices && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: services.some(s => s.image_url) ? "repeat(2,1fr)" : "1fr", gap: 12, marginBottom: 12 }}>
              {services.map(s => (
                <ServiceCard key={s.id} service={s} selected={selSvc === s.id} currency={profile?.currency} onClick={() => { setSelSvc(s.id); setCustom(false); }} />
              ))}
            </div>
