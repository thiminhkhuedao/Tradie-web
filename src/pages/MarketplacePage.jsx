...
// Safe (no-space) translation-key suffixes for material conditions
const CONDITION_KEY = { "New":"new", "Like new":"likeNew", "Used":"used", "For parts":"forParts" };

const TYPE_COLORS = {
  demand:      { bg: "#EFF6FF", text: "#1D4ED8", border: "#BFDBFE" },
  sale:        { bg: "#FEF3CD", text: "#92530A", border: "#FDE68A" },
  recruitment: { bg: "#EDFAF3", text: "#1A7F4B", border: "#BBF7D0" },
  materials:   { bg: "#F5F3FF", text: "#6D28D9", border: "#DDD6FE" },
};
...
export default function MarketplacePage({ profile }) {
  ...
  const load = useCallback(async () => {
    ...
    setListings(data ?? []);
    setLoading(false);
  }, [activeTab, filterTrade, filterUrgent]);
  ...
  return (
    <PageShell title={t("marketplace.title")}
      action={
    ...
    >
      ...
      {/* ── Delete confirm ── */}
      {delId && (
        <ConfirmModal title={t("marketplace.deleteConfirm.title")}
          message={t("marketplace.deleteConfirm.message")}
          confirmLabel={t("marketplace.deleteConfirm.confirmLabel")} onConfirm={handleDelete} onClose={()=>setDelId(null)}/>
      )}
      ...
      {view==="browse" && (
        <>
          ...
          <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
            <input style={{...iStyle,flex:1,minWidth:180}}
              placeholder={t("marketplace.searchPlaceholder")}
              value={search} onChange={e=>setSearch(e.target.value)}/>
            <select style={{...iStyle,width:"auto"}}
          ...
          {/* Listings grid */}
          {loading ? (
            <div style={{textAlign:"center",padding:48,color:T.muted}}>{t("marketplace.loading")}</div>
          ) : displayed.length===0 ? (
            <Empty icon="🗂️" message={t("marketplace.emptyBrowse")}
      ...
      {view==="mine" && (
        <div>
          ...
          {myListings.length===0 ? (
            <Empty icon="📋" message={t("marketplace.emptyMine")}
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                        ...
                        <Badge color={l.status==="active"?"green":"gray"}>{t(`marketplace.status.${l.status}`)}</Badge>
                      </div>
                    ...
                    <div style={{display:"flex",gap:6,marginLeft:16,flexShrink:0}}>
                      {l.status==="active" && (
                        <Btn size="sm" variant="ghost" onClick={()=>handleClose(l.id)}>{t("marketplace.close")}</Btn>
                      )}
      ...
      <div style={{ padding:"18px 20px" }}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <TypeBadge type={l.type}/>
            {l.urgent && <Badge color="red">🔥 {t("marketplace.urgent")}</Badge>}
            {l.condition && <Badge color="gray">{t(`marketplace.materialConditions.${CONDITION_KEY[l.condition] || l.condition}`)}</Badge>}
          </div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
          ...
          {l.trade && <span style={{...tagStyle}}>{getVerticalForProfession(l.trade).icon} {getProfessionLabel(l.trade, t)}</span>}
          ...
          {l.work_start_date && <span style={{...tagStyle}}>📅 {t("marketplace.fromDate", { date: fmtDate(l.work_start_date) })}</span>}
        </div>
...
══════════════════════════════════════════════════════ */
function ListingDetailModal({ listing: l, profile, onClose, onInterest }) {
  const { t } = useTranslation();
  ...
  return (
    <Modal title="" onClose={onClose} width={580}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        <TypeBadge type={l.type}/>
        {l.urgent && <Badge color="red">🔥 {t("marketplace.urgent")}</Badge>}
        {l.views>0 && <span style={{fontSize:12,color:T.hint,marginLeft:"auto"}}>👁 {t("marketplace.viewsCount", { count: l.views })}</span>}
      </div>
      ...
      {l.photos?.length > 0 && (
        <div style={{display:"flex",gap:8,marginBottom:16,overflowX:"auto"}}>
          {l.photos.map((url,i)=>(
            <img key={i} src={url} alt={`${l.title} ${t("marketplace.photoAlt")} ${i+1}`}
      ...
      <div style={{display:"flex",gap:12,fontSize:13,color:T.muted,marginBottom:16,flexWrap:"wrap"}}>
        ...
        {l.trade    && <span>{getVerticalForProfession(l.trade).icon} {getProfessionLabel(l.trade, t)}</span>}
        <span>🕐 {timeAgo(l.created_at, t)}</span>
      </div>

      {/* Description */}
      <div style={{fontSize:14,color:T.text,lineHeight:1.7,marginBottom:20,
        whiteSpace:"pre-wrap"}}>{l.description}</div>

      {/* Details grid */}
      <div style={{background:T.surface2,borderRadius:T.r.md,padding:"14px 18px",marginBottom:20}}>
        <DetailGrid listing={l}/>
      </div>

      {/* Contact info (shown only to logged-in users or if public) */}
      {l.contact_name && (
        <div style={{background:T.brandLight,borderRadius:T.r.md,
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 16px"}}>
      {rows.map(([label,value])=>(
        <div key={label}>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,
            textTransform:"uppercase",letterSpacing:"0.4px",marginBottom:2}}>{label}</div>
...
function PostModal({ profile, onClose, onPosted }) {
  ...
  async function submit(e) {
    ...
    if (!form.title || !form.description || !form.location) {
      toast.error(t("marketplace.toast.requiredTitleDescLocation")); return;
    }
    if (!form.contact_name || !form.contact_email) {
      toast.error(t("marketplace.toast.requiredContact")); return;
    }
    setBusy(true);
    const payload = {
      type,
    ...
    setBusy(false);
    if (error) { toast.error(t("marketplace.toast.failedPost")); return; }
    onPosted(data);
  }
  ...
  const TypeBtn = ({ id, icon }) => (
    <button type="button" onClick={()=>setType(id)} style={{
      ...
      <div style={{fontWeight:700,fontSize:14,color:type===id?T.brand:T.text}}>{t(`marketplace.post.typeCards.${id}.label`)}</div>
      <div style={{fontSize:12,color:T.muted,marginTop:3,lineHeight:1.4}}>{t(`marketplace.post.typeCards.${id}.desc`)}</div>
    </button>
  );
          ...
          <>
            ...
            <FieldRow>
              ...
              <Field label={t("marketplace.post.fields.locationLabel")} flex="1">
                <input style={iStyle} value={form.location} onChange={fld("location")}
              ...
              <>
                <FieldRow>
                  <Field label={t("marketplace.post.fields.businessTypeLabel")} flex="1">
                    <input style={iStyle} value={form.business_type} onChange={fld("business_type")}
                ...
                <FieldRow>
                  <Field label={t("marketplace.post.fields.contractTypeLabel")} flex="1">
                    <select style={iStyle} value={form.contract_type} onChange={fld("contract_type")}>
                      {CONTRACT_TYPES.map(ct=><option key={ct} value={ct}>{t(`marketplace.contractTypes.${ct}`)}</option>)}
                    </select>
                  </Field>
                  <Field label={t("marketplace.post.fields.salaryRangeLabel")} flex="1">
                    <input style={iStyle} value={form.salary_range} onChange={fld("salary_range")}
                      placeholder={t("marketplace.post.fields.salaryRangePlaceholder")}/>
                  </Field>
                </FieldRow>
                <Field label={t("marketplace.post.fields.experienceLabel")}>
                  <input style={iStyle} value={form.experience_req} onChange={fld("experience_req")}
                ...
                <FieldRow>
                  <Field label={t("marketplace.post.fields.categoryLabel")} flex="1">
                    <select style={iStyle} value={form.category} onChange={fld("category")}>
                      {MATERIAL_CATEGORIES.map(c=><option key={c} value={c}>{t(`marketplace.materialCategories.${c}`)}</option>)}
                    </select>
                  </Field>
                  <Field label={t("marketplace.post.fields.conditionLabel")} flex="1">
                    <select style={iStyle} value={form.condition} onChange={fld("condition")}>
                      {MATERIAL_CONDITIONS.map(c=><option key={c} value={c}>{t(`marketplace.materialConditions.${CONDITION_KEY[c]}`)}</option>)}
                    </select>
                  </Field>
                </FieldRow>
                <FieldRow>
            ...
            <label style={{display:"flex",alignItems:"center",gap:10,
              ...
              <input type="checkbox" checked={form.urgent} onChange={fld("urgent")}
          ...
          <>
            ...
            <FieldRow>
              ...
              <Field label={t("marketplace.post.contact.phone")} flex="1">
                <input style={iStyle} value={form.contact_phone} onChange={fld("contact_phone")}
            ...
            <div style={{background:T.surface2,borderRadius:T.r.md,
            ...
            <div style={{display:"flex",gap:10,justifyContent:"space-between",
              ...
              <Btn type="submit" disabled={busy}>
                {busy?t("marketplace.post.posting"):`🚀 ${t("marketplace.post.postBtn")}`}
              </Btn>
            </div>
          </>
        )}
