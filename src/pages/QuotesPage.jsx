...
export default function QuotesPage({ state, dispatch, profile, isLoading = false, error = null, onRetry }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (modal==="add") {
        ...
        await dispatch({ type:"ADD_QUOTE", payload:{ ...form, ...totals, id:uid(), quote_number:`QUO-${num}`, line_items, status:"draft", signed_at:null, job_id:null, created_at:new Date().toISOString() } });
        toast.success(tr("quotes.toast.quoteCreated"));
      } else {
        await dispatch({ type:"UPDATE_QUOTE", payload:{ ...form, ...totals, line_items } });
  ...
  async function markSent(id) {
    setBusy(true);
    try {
      await dispatch({ type:"UPDATE_QUOTE", payload:{ id, status:"sent" } });
  ...
  async function handleSign() {
    ...
    setBusy(true);
    try {
      await dispatch({ type:"UPDATE_QUOTE", payload:{ id:form.id, status:"accepted", signed_at:new Date().toISOString(), signed_by:signName } });
      setModal(null);
  ...
  async function convertToJob(q) {
    setBusy(true);
    try {
      await dispatch({ type:"ADD_JOB", payload:{ id:uid(), profile_id:profile?.id, client_id:q.client_id, title:q.title, date:today(), time:"09:00", duration:4, status:"scheduled", notes:`From quote ${q.quote_number}`, amount:q.total } });
      await dispatch({ type:"UPDATE_QUOTE", payload:{ id:q.id, status:"converted" } });
  ...
  async function handleDelete() {
    ...
    setBusy(true);
    try {
      await dispatch({ type:"DELETE_QUOTE", payload:delId });
      setDelId(null);
