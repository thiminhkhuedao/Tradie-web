...
export default function JobsPage({ profile }) {
  ...
  function openAdd() {
    setForm({ client_id:clients[0]?.id||"", title:"", date:today(),
              time:"09:00", duration:"2", amount:"", notes:"" });
    setModal("add");
  }
  function openEdit(job) {
    setForm({ ...job, client_id: job.client_id ?? job.client?.id ?? "",
              duration:String(job.duration), amount:String(job.amount) });
    setModal("edit");
  }
  ...
  async function save(e) {
    ...
    if (!form.title || !form.date || !form.client_id) {
      toast.error(t("jobs.errorRequired")); return;
    }
    setBusy(true);
    const payload = {
    ...
    if (modal==="add") {
      ...
      if (error) { toast.error(t("jobs.errorAdd")); setBusy(false); return; }
      setJobs(prev=>[data, ...prev]);
      toast.success(t("jobs.successAdd"));
    } else {
      const { data, error } = await updateJob(form.id, payload);
      if (error) { toast.error(t("jobs.errorUpdate")); setBusy(false); return; }
  ...
  async function handleComplete(id) {
    ...
    if (error) { toast.error(t("jobs.errorComplete")); return; }
  ...
  async function handleDelete() {
    ...
    if (error) { toast.error(t("jobs.errorDelete")); return; }
  ...
  return (
    <PageShell title={terms.bookingPlural} action={<Btn size="sm" onClick={openAdd}>+ {t("common.new")} {terms.booking.toLowerCase()}</Btn>}>
        ...
        <Modal
          ...
          <form onSubmit={save}>
            ...
            <Field label={`${terms.booking} ${t("jobs.titleLabel")}`}>
              <input style={iStyle} value={form.title} onChange={fld("title")}
                placeholder="e.g. Consumer unit replacement" autoFocus/>
            </Field>
            <FieldRow>
            ...
            <FieldRow>
              <Field label={t("jobs.duration")} flex="1">
                <input type="number" style={iStyle} value={form.duration}
                  onChange={fld("duration")} min="0.5" step="0.5"/>
              </Field>
            ...
            <Field label={t("jobs.notes")}>
              <textarea style={{...iStyle,height:72,resize:"vertical"}}
                value={form.notes} onChange={fld("notes")} placeholder={t("jobs.notesPlaceholder")}/>
            </Field>
            <FormActions onCancel={()=>setModal(null)}
              submitLabel={modal==="add"
                ? `${t("common.add")} ${terms.booking.toLowerCase()}`
                : t("common.save")}
              loading={busy}/>
          </form>
