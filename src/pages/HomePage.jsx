...
import { Modal, SmartInput, PasswordChecklist, SmartSubmitButton, isValidEmail, PASSWORD_RULES, formatPhoneNumber } from "../components/UI.jsx";
...
/* ── Auth Modal with 6 Form Usability Rules ─────── */
...
  // 4) Auto pre-fill saved email on mount ("Remember Me")
  useEffect(() => {
    const savedEmail = localStorage.getItem("Vimen_saved_email");
    if (savedEmail) {
      setEmail(savedEmail);
      setRememberMe(true);
    }
...
  // 6) Forgiving Phone Formatter on typing
  const handlePhoneChange = (e) => {
    setPhone(formatPhoneNumber(e.target.value));
  };

  // Validations & Missing Field Tracking
  const missingFields = [];
  
  if (!email || !isValidEmail(email)) missingFields.push("Valid Email");
  
  if (isSignUp) {
...
    if (!allPasswordRulesMet) missingFields.push("Strong Password");
  } else {
...
    // 4) Save email if "Remember Me" is checked
    if (rememberMe) {
      localStorage.setItem("Vimen_saved_email", email);
    } else {
      localStorage.removeItem("Vimen_saved_email");
    }

    // Simulate API call
    setTimeout(() => {
      setLoading(false);
      if (onSuccess) onSuccess({ email, fullName });
      onClose();
    }, 1200);
  };

  return (
    <Modal title={isSignUp ? t("auth.signUpTitle") : t("auth.signInTitle")} onClose={onClose} width={420}>
...
            maxLength={40}
            showCharCount={true} // 3) Char Count
            required
            error={!fullName.trim() ? "Full name is required" : null}
          />
        )}

        {/* 2) Dynamic Email Validation */}
        <SmartInput
          label="Email Address"
          type="email"
          value={email}
...
          required
          error={email && !isValidEmail(email) ? "Please enter a valid email address" : null}
        />

        {isSignUp && (
          /* 6) Forgiving Format Phone Field */
          <SmartInput
...
            />
            <label htmlFor="remember" style={{ fontSize: 13, color: T.muted, cursor: "pointer" }}>
              Remember my email on this device
            </label>
          </div>
        )}

        {/* 1) Disabled Submit Button + Missing Hint */}
        <SmartSubmitButton
...
            <Link to="/pricing" style={{ fontSize: 14, color: T.muted, textDecoration: "none", fontWeight: 500 }}> {t("home.nav.pricing")}</Link>
          </div>
        </div>
...
  return (
    <section style={{ padding: "80px 0", borderTop: `1px solid ${T.border}`, background: T.surface2 }}>
...
        </div>
        <h2 style={{ fontFamily: "'Archivo Black',sans-serif", fontSize: "clamp(26px,3.5vw,40px)", letterSpacing: -1, marginBottom: 16 }}>
          {t("home.pricingTeaser.title")}
        </h2>
        <p style={{ fontSize: 16, color: T.muted, maxWidth: 460, margin: "0 auto 32px", lineHeight: 1.7 }}>
          {t("home.pricingTeaser.sub")}
        </p>
        <a href="/pricing" style={{ display: "inline-block", background: T.brand, color: "#fff", padding: "13px 28px", borderRadius: 6, fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
...
  const [authModalMode, setAuthModalMode] = useState(null); // 'signin' | 'signup' | null

  const handleSignInClick = () => {
    if (onSignIn) onSignIn();
    else setAuthModalMode("signin");
  };
...
  return (
    <div style={S.page}>
      <NavBar onSignIn={handleSignInClick} onSignUp={handleSignUpClick} t={t} lang={lang} setLanguage={setLanguage} languages={languages} />
...
            <button onClick={handleSignUpClick} style={{ background: T.brand, color: "#fff", border: "none", padding: "14px 28px", borderRadius: 6, fontWeight: 700, fontSize: 15, cursor: "pointer" }}>
...
            {t("home.finalCta.sub")}
          </p>
          <button onClick={handleSignUpClick} style={{ background: T.brand, color: "#fff", border: "none", padding: "14px 32px", borderRadius: 6, fontWeight: 700, fontSize: 15, cursor: "pointer" }}>
            {t("home.finalCta.cta")}
          </button>
        </div>
      </section>
...
          <div style={{ fontFamily: "'Archivo Black',sans-serif", fontSize: 15 }}>Vimen</div>
          <div style={{ display: "flex", gap: 24 }}>
            <a href="#features" style={{ fontSize: 13, color: T.hint, fontWeight: 600, textDecoration: "none" }}>{t("home.nav.features")}</a>
            <Link to="/pricing" style={{ fontSize: 13, color: T.hint, fontWeight: 600, textDecoration: "none" }}> {t("home.nav.pricing")}</Link>
            <a href="mailto:hello@Vimen.app" style={{ fontSize: 13, color: T.hint, fontWeight: 600, textDecoration: "none" }}>{t("home.footer.contact")}</a>
          </div>
          <p style={{ color: T.hint, fontSize: 13 }}>{t("home.footer.copyright")}</p>
        </div>
      </footer>

      {/* Render Auth Modal when Sign In / Sign Up is triggered */}
      {authModalMode && (
        <AuthModal
          mode={authModalMode}
          onClose={() => setAuthModalMode(null)}
          onSuccess={(userData) => {
            console.log("Logged in / Registered:", userData);
          }}
        />
      )}
    </div>
